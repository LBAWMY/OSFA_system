...
def undistort_stereo(img_left, img_right):
    ...
    # T = np.array([[3.914635325710078], [0.1225452545407107], [0.2321917126623933]])
    # endregion
    ...
    # # cadaver2
    # cameraMatrixr = np.array([[611.7144025816513, 0, 318.9606832557497],
    #                           [0, 734.6089131949849, 237.1816823407249],
    #                           [0, 0, 1]
    #                           ])
    # distCoeffsr = np.array([-0.401105175367071, 0.1042483674512157, -0.001568033995046414, -0.0002098981712412147, 0.2260079561641118])
    #
    # cameraMatrixl = np.array([[617.2467717408903, 0, 313.8394120456613],
    #                           [0, 740.552160621133, 252.354116195112],
    #                           [0, 0, 1]
    #                           ])
    # distCoeffsl = np.array([-0.4327127849915921, 0.3909415935084647, -0.002552002184081203, -0.002248265639644834, -0.4323438771030861])
    #
    # R = np.array([[9.95423503e-01,  2.67798515e-03, -9.55242328e-02],
    #                [-6.54486865e-04,  9.99774868e-01,  2.12081331e-02],
    #                [ 9.55595223e-02, -2.10485547e-02,  9.95201154e-01]])
    # T = np.array([[3.88175623],[-0.08520211],[0.27793528]])
    ...
    return result1, result2, R1, R2, Q, map1_1  # cadaver setting
...
def undistort_job():
    global img_L_src, img_R_src, left, right, R1, R2, Q, map1_1   # cadaver setting
    # global img_L_src, img_R_src, left, right, R1, R2, Q, map2_1
    while not rospy.is_shutdown():
        if img_L_src is not None and img_R_src is not None:
            left, right, R1, R2, Q, map1_1 = undistort_stereo(img_L_src, img_R_src)  # cadaver setting
            # left, right, R1, R2, Q, map2_1 = undistort_stereo(img_L_src, img_R_src)
#
# def caldepth_job():
        ...
        try:
            with torch.no_grad():
                ...
                for i, det in enumerate(pred):  # detections per image
                    ...
                    #     # tool_depth = tool_depth_in_thread # SteroTempleteDepthCal_left(rightImg=right, l_bbox=l_bbox, leftImg=left, resize_factor=scale, R1=R1, Q=Q)
                    ...
                    for i in range(NUM_TOOLS_TO_TRACK):
                        if (save_img or view_img) and len(tracked_dets[i]) > 0:  # Add bbox to image + Add trajectory to image
                            ...
                            objects[i].plot_undistort(L_Img, color=colors[i], map=map1_1)   # cadaver setting
                            # objects[i].plot_undistort(L_Img, color=colors[i], map=map2_1)
                    ...
                    if view_img:
                        ...
                        print('------total time in a loop------: {}, dist: {}, pre-process: {}, yolo: {}, track: {}, '
                              'depth: {}, plot_o: {}, publish: {}, view: {}'.format
                              (t_end - t_start, t_dist-t_start, t1-t_dist, t2-t1, t3-t2, t4-t3, time_publish_start-t4,
                               time_publish_end-time_publish_start, t_end - time_publish_end))
