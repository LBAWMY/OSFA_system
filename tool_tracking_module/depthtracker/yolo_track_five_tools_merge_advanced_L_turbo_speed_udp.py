...
from udp.curi_udp import curi_udp
import threading
...
def SteroTempleteDepthCal_left(rightImg, l_bbox, leftImg, R1, Q):
    ...
    scale_ = np.minimum(50.0/(l_bbox[2] - l_bbox[0]), 50.0/(l_bbox[3] - l_bbox[1]))
    ...
    if (len(l_bbox) != 0 and min(l_bbox) > 0 and max([l_bbox[0], l_bbox[2]]) <= width and max([l_bbox[1], l_bbox[3]]) <= height):
        ...
        # cv2.imshow('Matched LeftImg', rightImg_[l_box[1]:l_box[3], u_r:u_r+(l_box[2]-l_box[0]),])

        if start_index + u_r + (l_box[2]-l_box[0]) <= size[0] * 0.1: # if there is no satified matching in right image
        ...
        try:
            with torch.no_grad():
                ...
                for i, det in enumerate(pred):  # detections per image
                    ...
                    # udp publish
                    if len(objects[3].undistorted_center) > 0 and len(tracked_dets[3]) > 0:
                        CS.send(str(objects[3].undistorted_center[0, 0]) + "#" + str(objects[3].undistorted_center[0, 1]) + "#" + str(-1))
                    else:
                        CS.send(str(-1) + "#" + str(-1) + "#" + str(-1))
