...
import sys
sys.path.append('../')
from yolov5_obb.utils.datasets import LoadImages, LoadStreams, LoadRosImages
from yolov5_obb.utils.general import check_img_size, non_max_suppression, scale_coords, xyxy2xywh
...
from yolov5_obb.utils.plots import plot_one_box
...
img_LR_src = None
main_tool_id = 4 # the default main tool index
...
def undistort_stereo(img_left, img_right):
    ...
    # cameraMatrixl = np.array([[675.803241669486,	0,	350.843002365452],
    #                           [0, 676.825319702307, 244.694254393841],
    #                           [0, 0, 1]
    #                           ])
    # distCoeffsl = np.array([-0.421512608845406,	0.247574517258580, 0.0, 0.0, 0.0])
    #
    # cameraMatrixr = np.array([[684.797064740665, 0, 333.929700995711],
    #                           [0, 671.657864471941, 231.504802226578],
    #                           [0, 0, 1]
    #                           ])
    # distCoeffsr = np.array([-0.420773526710595,	0.282788773596518, 0.00, 0.00, 0.0])
    #
    # # R = np.array([[0.999673841631948,	0.000229842928980489,	-0.0255373751401865],
    # #               [-0.000455580255276006,	0.999960875487795,	-0.00883402175192188],
    # #               [0.0255343455654070,	0.00884277478568939,	0.999634834592431]])
    # # T = np.array([[-3.97863493660047], [-0.0980121965265191], [-0.0651474821652640]])
    # R = np.array([[ 9.99673842e-01, -4.55580255e-04,  2.55343456e-02],
    #              [2.29842929e-04,  9.99960875e-01,  8.84277479e-03],
    #              [-2.55373751e-02, -8.83402175e-03,  9.99634835e-01]])
    # T = np.array([[3.97895612], [9.94989075e-02], [-3.73460422e-02]])

    # cameraMatrixl = np.array([[695.1812993513444, 0, 328.1569464883446],
    #                           [0, 659.5405317338548, 243.5077997593637],
    #                           [0, 0, 1]
    #                           ])
    # distCoeffsl = np.array([-0.3919505211360785, 0.09699625281142653, 0.0006215100336428928, 0.001103875631851141, 0.2645394746829857])
    #
    # cameraMatrixr = np.array([[705.4002860308384, 0, 355.3027457646199],
    #                           [0, 655.3836887747772, 232.1351898457214],
    #                           [0, 0, 1]
    #                           ])
    # distCoeffsr = np.array([-0.3820694847525396, 0.01399839245427121, -0.0002114564449689529, -0.001571865735764858, 0.4086517934364665])
    #
    # R = np.array([[0.9999234791144732, -0.0002180270457663032, 0.01236884714979857],[0.0001396387096741508, 0.9999799044722043, 0.006338071693499196],[-0.01236998046233405, -0.006335859528784524, 0.9999034155694204]])
    # T = np.array([[-4.014896157109533],[-0.0982298700819661],[0.1011995650909303]])

    # cameraMatrixl = np.array([[657.7589620923933, 0, 344.4426665934281],
    #                           [0, 658.7316177523766, 247.6107850860998],
    #                           [0, 0, 1]
    #                           ])
    # distCoeffsl = np.array([-0.4044887445783718, 0.2633663820313435, -0.001030491186084815, 0.001492002281917136, -0.09272422281363203])
    #
    # cameraMatrixr = np.array([[668.4289599518221, 0, 333.2768362461701],
    #                           [0, 655.7211920251866, 233.5703747964841],
    #                           [0, 0, 1]
    #                           ])
    # distCoeffsr = np.array([-0.3968808329364075, 0.2110336932462835, -0.002849256488364839, -0.0009463274932231365, -0.03090328376994649])
    #
    # R = np.array([[0.9998506795092658, 2.603511743127804e-05, 0.01728056732379886],[-0.0002058586068784693, 0.9999458514395491, 0.0104044130574085],[-0.01727936072583875, -0.01040641681886031, 0.9997965443938575]])
    # T = np.array([[-3.92847865172259],[-0.08314327660762448],[0.3410647022066308]])

    # cameraMatrixl = np.array([[660.9719924080497, 0, 343.4565069258681],
    #                           [0, 661.973585822014, 243.0896141083768],
    #                           [0, 0, 1]
    #                           ])
    # distCoeffsl = np.array([-0.418255644631139, 0.3512638242957011, -0.002010704791154946, 0.001303398363815274, -0.3745199661978111])
    #
    # cameraMatrixr = np.array([[669.3664619527359, 0, 339.2945746088841],
    #                           [0, 656.547095251917, 232.4834160094292],
    #                           [0, 0, 1]
    #                           ])
    # distCoeffsr = np.array([-0.3754327089071072, 0.0726069643042227, -0.0001420065049151235, -0.001362347629406722, 0.226433300754782])
    #
    # R = np.array([[0.9999751920764263, 0.0004578920034743423, 0.007028909348363976],
    #               [-0.0004798205466640777, 0.9999950226786507, 0.00311839544766036],
    #               [-0.007027446474884261, -0.003121690701870402, 0.9999704345845454]])
    # T = np.array([[-4.002117869355506],[0.1700615524025405],[0.02667007756786813]])
...
def SteroTempleteDepthCal_right(rightImg, r_bbox, leftImg, resize_factor, R2, Q):
    ...
    if (len(r_bbox) != 0 and min(r_bbox) > 0 and max([r_bbox[0], r_bbox[2]]) <= width and max([r_bbox[1], r_bbox[3]]) <= height):
        ...
        tool_depth = xyz_rectified[2]
        # tool_depth = 1
        print('************************************** disparity/depth: {}/{}'.format(visual_d, tool_depth))
    ...
    while not rospy.is_shutdown():
        try:
            with torch.no_grad():
                ...
                img = imgR # use the right frame for detection
                ...
                for i, det in enumerate(pred):  # detections per image
                    ...
                    for x1, y1, x2, y2, conf, detclass in det.cpu().detach().numpy():
                        ...
                        # if int(detclass) == 5:
                        #     dets_to_track3 = np.vstack((dets_to_track3, np.array([x1, y1, x2, y2, conf])))
                        # if int(detclass) == 7:
                        #     dets_to_track4 = np.vstack((dets_to_track4, np.array([x1, y1, x2, y2, conf])))
                        # if int(detclass) == 9:
                        #     dets_to_track5 = np.vstack((dets_to_track5, np.array([x1, y1, x2, y2, conf])))
