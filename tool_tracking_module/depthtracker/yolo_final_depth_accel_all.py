...
xyz_error_pub = rospy.Publisher("/control/xyz_error", Vector3, queue_size=1)
...
def detect(opt, *args):
    ...
    tools_depth = [KalmanDepthTracker() for _ in range(NUM_TOOLS_TO_TRACK)]
    # frame count
    frame_count = 1
        ...
        try:
            with torch.no_grad():
                ...
                for i, det in enumerate(pred):  # detections per image
                    ...
                    for i in range(NUM_TOOLS_TO_TRACK):
                        l_bbox = trackers[i].cur_bbox[:4]
                        if trackers[i].cur_bbox_flag is True: #  and system_version >= 2
                            # current: 0.001s
                            tool_depth = StereoDepth_left(rightImg=right, l_bbox=l_bbox, leftImg=left, R1=R1, Q=Q)
                        else:
                            print('no valid depth estimation ! The current bbox is the previous one.')
                            tool_depth = -1
                        _, _ = tools_depth[i].update(tool_depth)

                    # extract the main tool depth
                    filter_depth = tools_depth[main_tool_id].filter_depth
                    print('Filtered depth: ', filter_depth)
                    ...
                    for i in range(NUM_TOOLS_TO_TRACK):
                        ...
                        if len(objects[i].undistorted_center) > 0 and len(tracked_dets[i]) > 0:
                            ...
                            pred_position_msg = Vector3(objects[i].undistorted_center[0, 0], objects[i].undistorted_center[0, 1], tools_depth[i].filter_depth)
                        else:
                            pred_position_msg = Vector3(-1, -1, -1)
                        pred_position_tools_pub[i].publish(pred_position_msg)

                    # publish the raw pixel error in xy direction and the depth error in z direction
                    xyz_error_msg = Vector3(x_error, y_error, z_error)
                    xyz_error_pub.publish(xyz_error_msg)
