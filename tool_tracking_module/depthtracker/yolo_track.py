...
#kalman filter
from KalmanFilter import convert_bbox_to_z, convert_x_to_bbox, KalmanBoxTracker, Tracker, Object
...
def detect(opt, *args):
    ...
    colors = [[0, 0, 0], [0, 255, 0], [0, 0, 0], [0, 0, 0], [0, 0, 255]]
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            for x1, y1, x2, y2, conf, detclass in det.cpu().detach().numpy():
                dets_to_track = np.vstack((dets_to_track, np.array([x1, y1, x2, y2, conf])))
            ...
            tracked_dets = tracker.update(dets_to_track)
            # print('tracked dets: ', tracked_dets)
            tracker.update_bbox_heatmap(tracked_dets)
            object.update(tracked_dets)
            ...
            if (save_img or view_img) and len(tracked_dets) > 0:  # Add bbox to image + Add trajectory to image
                label = f'{names[int(4)]} {1.0:.2f}'
                plot_one_box(tracked_dets[:4], im0, label=label, color=colors[int(1)], line_thickness=3)
                object.plot(im0)
