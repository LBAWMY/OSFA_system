            ...
            with torch.no_grad():
                ...
                for i, det in enumerate(pred):  # detections per image
                    ...
                    for x1, y1, x2, y2, conf, detclass in det.cpu().detach().numpy():
                        dets_to_track[int(detclass) // 2] = np.vstack((dets_to_track[int(detclass) // 2], np.array([x1, y1, x2, y2, conf])))
                        # if int(detclass) == 1:
                    ...
                    if view_img:
                        ...
                        # visual test for disparity
                        disparity = 0
                        cv2.putText(R_Img, filter_tool_depth_, (100, 130), 0, 1, [225, 255, 0], thickness=2,
                                    lineType=cv2.LINE_AA)
                        cv2.putText(L_Img, filter_tool_depth_, (100+disparity, 130), 0, 1, [225, 255, 0], thickness=2,
                                    lineType=cv2.LINE_AA)
                        cv2.putText(R_Img, filter_tool_depth_, (140, 250), 0, 1, [225, 255, 0], thickness=2,
                                    lineType=cv2.LINE_AA)
                        cv2.putText(L_Img, filter_tool_depth_, (140+disparity, 250), 0, 1, [225, 255, 0], thickness=2,
                                    lineType=cv2.LINE_AA)
                        cv2.putText(R_Img, filter_tool_depth_, (180, 370), 0, 1, [225, 255, 0], thickness=2,
                                    lineType=cv2.LINE_AA)
                        cv2.putText(L_Img, filter_tool_depth_, (180+disparity, 370), 0, 1, [225, 255, 0], thickness=2,
                                    lineType=cv2.LINE_AA)
                        
                        # right_vis = cv2.resize(right, (1920, 1080))
                        # left_vis = cv2.resize(left, (1920, 1080))

                        # cv2.imshow('R_Img_Dis', right_vis)
                        # cv2.imshow('L_Img_Dis', left_vis)
