...
def non_max_suppression_obb(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    Args:
        prediction (tensor): (b, n_all_anchors, [cx cy l s obj num_cls theta_cls topleft_cls])
        agnostic (bool): True = NMS will be applied between elements of different categories
        labels : () or

    Returns:
        list of detections, len=batch_size, on (n,7) tensor per image [cx, cy, l, s, θ, topleft, conf, cls] θ ∈ [-pi/2, pi/2)
    """

    nc = prediction.shape[2] - 5 - 180 - 4  # number of classes
    xc = prediction[..., 4] > conf_thres  # candidates
    class_index = nc + 5
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        # Apply constraints
        # x[((x[..., 2:4] < min_wh) | (x[..., 2:4] > max_wh)).any(1), 4] = 0  # width-height
        x = x[xc[xi]]  # confidence, (tensor): (n_conf_thres, [cx cy l s obj num_cls theta_cls topleft_cls])
        ...
        _, theta_pred = torch.max(x[:, class_index:180+class_index], 1,  keepdim=True) # [n_conf_thres, 1] θ ∈ int[0, 179]
        theta_pred = (theta_pred - 90) / 180 * pi # [n_conf_thres, 1] θ ∈ [-pi/2, pi/2)

        _, topleft_pred = torch.max(x[:, 180+class_index:], 1,  keepdim=True) # [n_conf_thres, 1] tl ∈ int[0, 3]

        # Detections matrix nx7 (xyls, θ, conf, cls) θ ∈ [-pi/2, pi/2)
        if multi_label:
            i, j = (x[:, 5:class_index] > conf_thres).nonzero(as_tuple=False).T # ()
            x = torch.cat((x[i, :4], theta_pred[i], topleft_pred[i], x[i, j + 5, None], j[:, None].float()), 1)
        else:  # best class only
            conf, j = x[:, 5:class_index].max(1, keepdim=True)
            x = torch.cat((x[:, :4], theta_pred, topleft_pred, conf, j.float()), 1)[conf.view(-1) > conf_thres]
