...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # NMS
        # pred: list*(n, [cxcylsθ, conf, cls]) θ ∈ [-pi/2, pi/2), i.e. (1, 15120, 197)
        # After non_max_suppression_obb, list*(2, 8) [cx cy l s theta_cls topleft_cls conf class]
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                pred_topleft = label2order(theta=det[:, 4], label=det[:, 5])  # [1,2,3,4]

                pred_poly = scale_polys(im.shape[2:], pred_poly, im0.shape)
                det = torch.cat((pred_poly, pred_topleft, det[:, -2:]), dim=1) # (n, [poly topleft conf class])
                ...
                # Write results
                for *poly, topleft, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add poly to image
                        ...
                        annotator.poly_label(poly, topleft, label, color=colors(c, True))
