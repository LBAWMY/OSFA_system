...
# kalman filter
from KalmanFilter_multi import convert_bbox_to_z, convert_x_to_bbox, KalmanBoxTracker, Tracker, Object, xywh2xyxy
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    object1 = Object(num=15)
    object2 = Object(num=15)
    object3 = Object(num=15)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            # tracked_topleft1 = dets_to_topleft1[-1, 0] if dets_to_topleft1.shape[0] > 0 else 1
            # _, tracked_topleft1 = tracker1_TopLeft.update(dets_to_topleft1, tracked_dets1[0, 4])
            ...
            # tracked_topleft2 = dets_to_topleft2[-1, 0] if dets_to_topleft2.shape[0] > 0 else 1
            # _, tracked_topleft2 = tracker2_TopLeft.update(dets_to_topleft2, tracked_dets2[0, 4])
            tracker2.update_bbox_heatmap(tracked_dets2)
            object2.update(tracked_dets2)

            m3, tracked_dets3 = tracker3.update(dets_to_track3)
            # tracked_topleft3 = dets_to_topleft3[-1, 0] if dets_to_topleft3.shape[0] > 0 else 1
            # _, tracked_topleft3 = tracker3_TopLeft.update(dets_to_topleft3, tracked_dets3[0, 4])
            ...
            if (save_img or view_img) and len(tracked_dets1) > 0:  # Add bbox to image + Add trajectory to image
                ...
                annotator.poly_label(tracked_poly1, tracked_topleft1, label, color=colors(int(0), True))
            ...
            if (save_img or view_img) and len(tracked_dets2) > 0:  # Add bbox to image + Add trajectory to image
                ...
                annotator.poly_label(tracked_poly2, tracked_topleft2, label, color=colors(int(2), True))
            ...
            if (save_img or view_img) and len(tracked_dets3) > 0:  # Add bbox to image + Add trajectory to image
                ...
                annotator.poly_label(tracked_poly3, tracked_topleft3, label, color=colors(int(4), True))
