...
class CustomJoy(QWidget):
    def __init__(self, parent=None):
        super(CustomJoy, self).__init__(parent)
        self.m_pressIndex = 0
        self.m_enterIndex = 0
        self.m_isMouseEntered = False
        self.m_isMousePressed = False
        self.m_scaledSize = 200.0
        self.m_radius = 80
        self.m_arcLength = 55
        ...
        self.initButton()
        self.setWidgetStyle("Bright") # Dark
    ...
    def paintEvent(self, QPaintEvent):
        ...
        painter.scale(realSize / self.m_scaledSize, realSize / self.m_scaledSize)
    ...
    def mouseMoveEvent(self, event):
        if (self.m_isMousePressed):
            ...
            EPS = 1e-6
            if (x > 0 and x < EPS) or (x < 0 and x > -EPS):
                x = 0
            if (y > 0 and y < EPS) or (y < 0 and y > -EPS):
                y = 0
    ...
    def analysisAngle(self, x, y):
        # x = max(1e-6, x) if x > 0 else min(-1e-6, x)
        # y = max(1e-6, y) if y > 0 else min(-1e-6, y)
        # if x > 0:
        #     x = max(1e-6, x)
        # elif x < 0:
        #     x = min(-1e-6, x)
        ...
        if (y > 0 and y < EPS) or (y < 0 and y > -EPS):
            y = 0
        angle = 180 - math.atan2(y, -x) / (2 * math.acos(-1)) * 360
        angle = min(angle, 359)
        print(x, y, 'end', angle)
        return angle
