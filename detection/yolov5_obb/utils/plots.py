...
class Annotator:
    ...
    def poly_label(self, poly, top_left, label='', color=(128, 128, 128), txt_color=(255, 255, 255), topleft_color=(255,255,0)):
        if self.pil or not is_ascii(label):
            ...
            if isinstance(top_left, torch.Tensor):
            ...
            if top_left is not None and top_left >= 1:
                cv2.circle(img=self.im, center=(polygon_list[top_left-1, 0], polygon_list[top_left-1, 1]), radius=5, color=topleft_color, thickness=9) # thickness=self.lw
            if label:
                tf = max(self.lw - 1, 2)  # font thicknes
                xmax, xmin, ymax, ymin = max(poly[0::2]), min(poly[0::2]), max(poly[1::2]), min(poly[1::2])
                x_label, y_label = int((xmax + xmin)/2), int((ymax + ymin)/2)
                w, h = cv2.getTextSize(label, 0, fontScale=self.lw / 3 * 2, thickness=tf)[0]  # text width, height
                cv2.rectangle(
                                self.im,
                                (x_label, y_label),
                ...
                cv2.putText(self.im, label, (x_label, y_label + h), 0, self.lw / 3 * 2, txt_color, thickness=tf, lineType=cv2.LINE_AA)

    def point(self, xy, color=(128, 128, 128)):
        # Add the points to image
        cv2.circle(img=self.im, center=xy, radius=self.lw, color=color, thickness=self.lw)
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=2048, max_subplots=4):
    ...
    Args:
        ...
        targets_train (tensor): (n_targets, [batch_id clsid cx cy l s theta gaussian_θ_labels topleft_labels]) θ∈[-pi/2, pi/2)
    ...
    for i in range(i + 1):
        ...
        if len(targets) > 0:
            ti = targets[targets[:, 0] == i]  # image targets, (n, [img_index clsid cx cy l s theta gaussian_θ_labels topleft_labels])
            # boxes = xywh2xyxy(ti[:, 2:6]).T
            rboxes = ti[:, 2:7]
            classes = ti[:, 1].astype('int')
            # labels = ti.shape[1] == 6  # labels if no conf column
            labels = ti.shape[1] == 187+4  # labels if no conf column
