...
def poly2rbox_order(polys, num_cls_thata=180, radius=6.0, use_pi=False, use_gaussian=False):
    """
    Trans poly format to rbox format.
    Args:
        polys (array): (num_gts, [x1 y1 x2 y2 x3 y3 x4 y4])
    ...
    for poly in polys:
        ...
        ################################################################## calculate the topleft list order here
        ################################################################## recalculate the position for four corners
        ...
        box_array = np.concatenate([point1, point2, point3, point4], axis=0)
        dist = [np.linalg.norm(box_array[index % 4, :] - poly[0, :]) + np.linalg.norm(box_array[(index + 1) % 4, :] - poly[1, :]) +
                np.linalg.norm(box_array[(index + 2) % 4, :] - poly[2, :]) + np.linalg.norm(box_array[(index + 3) % 4, :] - poly[3, :]) for index in range(4)]
        topleft_index = np.argmin(dist)

        order = topleft_index + 1  # point 1~4

        label = order2label(theta, order)
        tl_label = np.zeros((4,))
        tl_label[label] = 1
        tl_labels.append(tl_label)

    if use_gaussian:
        return np.array(rboxes), np.array(csl_labels), np.array(tl_labels)
    return np.array(rboxes), np.array(tl_labels)

def order2label(theta, order):
    """
    convert the point number (1,2,3,4 - line129~137)
    into the direction (0, 1, 2, 3) according to the theta
    """
    if theta < 0:
...
def label2order(theta, label):
    ...
    #     else:  # label == 3:
    #         order = 1
    order = torch.zeros((label.shape[0], 1), dtype=torch.int, device=label.device)
    order[torch.bitwise_and(theta < 0, label == 0)] = 1
    order[torch.bitwise_and(theta < 0, label == 1)] = 2
    order[torch.bitwise_and(theta < 0, label == 2)] = 3
    order[torch.bitwise_and(theta < 0, label == 3)] = 4

    order[torch.bitwise_and(theta >= 0, label == 0)] = 2
    order[torch.bitwise_and(theta >= 0, label == 1)] = 3
    order[torch.bitwise_and(theta >= 0, label == 2)] = 4
    order[torch.bitwise_and(theta >= 0, label == 3)] = 1
