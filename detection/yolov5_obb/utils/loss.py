...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        BCEtopleft = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['topleft_pw']], device=device))
        ...
        if g > 0:
            ...
            BCEtopleft = FocalLoss(BCEtopleft, g)
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        ltopleft = torch.zeros(1, device=device)
        # tcls, tbox, indices, anchors = self.build_targets(p, targets)  # targets
        tcls, tbox, indices, anchors, tgaussian_theta, ttopleft_index = self.build_targets(p, targets)  # targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                ltheta += self.BCEtheta(ps[:, class_index:-4], t_theta)

                # top-left Classification
                t_topleft = ttopleft_index[i].type(ps.dtype)
                ltopleft += self.BCEtopleft(ps[:, -4:], t_topleft)
        ...
        ltopleft *= self.hyp['topleft']
        bs = tobj.shape[0]  # batch size

        # return (lbox + lobj + lcls) * bs, torch.cat((lbox, lobj, lcls)).detach()
        return (lbox + lobj + lcls + ltheta + ltopleft) * bs, torch.cat((lbox, lobj, lcls, ltheta, ltopleft)).detach()

    def build_targets(self, p, targets):
        """
        Args:
            p (list[P3_out,...]): torch.Size(b, self.na, h_i, w_i, self.no), self.na means the number of anchors scales
            targets (tensor): (n_gt_all_batch, [img_index clsid cx cy l s theta gaussian_θ_labels]) pixel: TODO: Bin LI, add the first point order
        ...
        """
        # explanation: https://gitcode.net/mirrors/laughing-q/yolov5_annotations/-/blob/master/utils/loss.py
        # explanation: https://codeantenna.com/a/yrIdVpp6rP
        ...
        for i in range(self.nl):
            ...
            gwh = t[:, 4:6]  # grid wh
            # theta = t[:, 6]
            gaussian_theta_labels = t[:, 7:-5]
            top_left_labels = t[:, -5:-1]
            ...
            ttopleft_index.append(top_left_labels)

        # return tcls, tbox, indices, anch
        return tcls, tbox, indices, anch, tgaussian_theta, ttopleft_index #, ttheta
