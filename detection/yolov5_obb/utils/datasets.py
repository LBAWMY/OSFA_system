    ...
    def __getitem__(self, index):
        ...
        #                                     use_pi=True, use_gaussian=True)
            rboxes, csl_labels, tl_labels = poly2rbox_order(polys=labels[:, 1:],
                                           ...
                                           radius=hyp['csl_radius'] if hyp else 6.0,
                                           use_pi=True, use_gaussian=True)
            labels_obb = np.concatenate((labels[:, :1], rboxes, csl_labels, tl_labels), axis=1)
        ...
        if hyp:
            c_num = 7 + hyp['cls_theta'] + 4 # [index_of_batch clsid cx cy l s theta gaussian_θ_labels topleft_labels]
        else:
            c_num = 187 + 4
