...
# capture image
rospy.Subscriber("/camera/usb_cam/image_raw", Image, imageCallback, queue_size=1, buff_size=2**24)
    ...
    while not rospy.is_shutdown():
        try:
            with torch.no_grad():
                ...
                W, H = img_src.shape[:2]
                # Padded resize
                img = letterbox(img_src, 640, 32)[0]
                ...
                for i, det in enumerate(pred):  # detections per image
                    ...
                    if (save_img or view_img) and len(tracked_dets1) > 0:  # Add bbox to image + Add trajectory to image
                        ...
                        object1.plot_distort(im0, color=colors[int(1)])
                    if (save_img or view_img) and len(tracked_dets2) > 0:  # Add bbox to image + Add trajectory to image
                        ...
                        object2.plot_distort(im0, color=colors[int(3)])
                    ...
                    if len(object1.distorted_center) > 0 and len(tracked_dets1) > 0:
                        pred_position_aux_msg = Vector3(object1.distorted_center[0, 0], object1.distorted_center[0, 1], -1)
                    else:
                        pred_position_aux_msg = Vector3(-1, -1, -1)

                    if len(object2.distorted_center) > 0 and len(tracked_dets2) > 0:
                        pred_position_main_msg = Vector3(object2.distorted_center[0, 0], object2.distorted_center[0, 1], -1)
                    else:
                        pred_position_main_msg = Vector3(-1, -1, -1)
